...
<head>
<meta charset="UTF-8">
<title>PKTDR Booking System</title>
...
table th, table td { padding:10px 12px; border-bottom:1px solid #ccc; }
...
select {
    ...
    background:#fff; color:#333; font-size:15px; margin-bottom:25px;
}
select:focus { outline:none; border-color:var(--main-orange); box-shadow:0 0 8px rgba(255,60,0,0.3); }

button {
    display:block; width:100%; padding:12px;
    background: linear-gradient(45deg,var(--main-orange),var(--hover-orange));
    border:none; border-radius:8px; color:#fff; font-weight:600; font-size:16px;
...
<div class="container">
    ...
    <table>
        <tr><th>Facility</th><td>{{ $booking->facility }}</td></tr>
        <tr><th>Date</th><td>{{ $booking->booking_date }}</td></tr>
        <tr><th>Time</th><td>{{ \Carbon\Carbon::parse($booking->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($booking->end_time)->format('H:i') }}</td></tr>
        <tr><th>Duration</th><td>{{ $booking->duration }} hour(s)</td></tr>
        <tr><th>Total</th><td>MYR {{ number_format($booking->amount, 2) }}</td></tr>
    </table>

    <form action="{{ route('customer.payment.redirect') }}" method="POST">
    @csrf
